...
if ($tab == 'print_invoice') {
	...
	$file_name = $mysqli->generateInvoice($id, isset($rows['membership_type']) ? $rows['membership_type'] : '');
	if($file_name){
		echo APPLICATION_URL.'backend/download_invoice.php?file='.urlencode($file_name);
	}else{
		echo 'ERROR: Invoice PDF could not be generated. Please check Dompdf image/GD configuration and invoice folder permission.';
	}
}
